package com.example.SocialNettwork.UserInterface;
...
import com.example.SocialNettwork.Domain.validators.ValidationException;
...
public class UserInterface {
    ...
    public UserInterface(UserService us, FriendshipService fs){
        userService=us;
    ...
    public void importEntities(){
        try {
            ...
            friendshipService.addFriendship(new Friendship(t1));
            friendshipService.addFriendship(new Friendship(t2));
            friendshipService.addFriendship(new Friendship(t4));
            friendshipService.addFriendship(new Friendship(t5));
        }
    ...
    public void addUser(){
        try {
            ...
            System.out.println(userService.addUser(new User(first_name, last_name)).toString());
        }
        catch (ValidationException | IllegalArgumentException ve){
            System.out.println(ve.getMessage());
        } catch (Throwable e){
    ...
    public void deleteUser(){
        try {
            Iterable<User>lista= userService.getAll();
            ...
            if(0<=numberToDellete && numberToDellete<lu.size()){
                friendshipService.deleteInCascade(lu.get(numberToDellete).getID());
                System.out.println(userService.deleteUser(lu.get(numberToDellete).getID()).toString());
            }
    ...
    public void showAllFforU(){
        try {
            ...
            if(0<=numberInt && numberInt<lu.size()){
                ...
                List<Long> friendsid=userService.getFrinedsList(user.getID());
                for(int i=0;i<friendsid.size();i++){
                    User u = userService.getUser(friendsid.get(i));
        ...
        try {
            ...
            nr2Int>=0 && nr2Int<lista.size()) {
                ...
                if (!Objects.equals(id1, id2)) {
                    ...
                    friendshipService.addFriendship(f).toString();
                }
            }
    ...
    public void deleteFriendship(){
        try{
            Iterable<Friendship>lista= friendshipService.getAll();
            Vector<Friendship> lf = new Vector<>();
            lista.forEach(lf::add);
            for(int i=0;i<lf.size();i++){
                System.out.println(i+"->"+friendshipService.friendshipToString(lf.get(i)));
            }
            ...
            if(0<=numberToDellete && numberToDellete<lf.size()){
                friendshipService.deleteFriendship(lf.get(numberToDellete).getID()).toString();
    ...
    public void showAllF(){
        try{
            ...
            ls.forEachRemaining(f->System.out.println(String.valueOf(nr.getAndIncrement())+"-"+friendshipService.friendshipToString(f)));
        }
        catch (ValidationException | IllegalArgumentException ve){
            System.out.println(ve.getMessage());
        }
